...
export default function Basket({ changeOpen, isOpen }) {
	return (
		<div
			className={`${isOpen ? "w-screen bg-black/70" : "w-0 bg-black/0"} fixed top-0 left-0 transition-colors duration-500 h-screen flex justify-end`}
		>
			<div
				className={`${isOpen ? "transform translate-x-0" : "transform -translate-x-72"} p-5 h-screen w-72 bg-white transition-transform duration-500`}
			>
				<button
